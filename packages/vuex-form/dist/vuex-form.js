...
var vuexToolkit = require('@ttungbmt/vuex-toolkit');
var VueFormulate = _interopDefault(require('@braid/vue-formulate'));
var veeValidate = require('vee-validate');
var rules = require('vee-validate/dist/rules');
var lodashEs = require('lodash-es');
var uniqid = _interopDefault(require('uniqid'));
var FormulateInputMixin = _interopDefault(require('@braid/vue-formulate/src/FormulateInputMixin'));
var vSelect = _interopDefault(require('vue-select'));
...
var script = {
  ...
  computed: {
    ...
    computedAttrs: function computedAttrs() {
      ...
      if (attrs.type === 'm-radio') {
        if (lodashEs.isUndefined(attrs.inline)) attrs.elementClass = ['flex gap-4 pt-2'];
        if (lodashEs.isString(attrs.items)) attrs.items = this.$store.getters['form/getCat'](attrs.items).map(function (v) {
          return _extends({
            id: uniqid()
          }, v);
        });
    ...
    errorBehavior: function errorBehavior() {
      return 'blur';
    },
    modelValue: {
      get: function get() {
        if (this.model) {
          var _this$getFormConfig = this.getFormConfig(),
              index = _this$getFormConfig.index;
  ...
  created: function created() {
    if (this.model) {
      var _this$getFormConfig2 = this.getFormConfig(),
          index = _this$getFormConfig2.index;
  ...
  methods: {
    onValidation: function onValidation(err) {
      this.$refs.provider.setErrors(Object.assign([], err.errors));
    },
    onShownValidation: function onShownValidation(val) {
      this.shownError = val;
    },
    debouncedModel: lodashEs.debounce(function () {
      var _this$getFormConfig3 = this.getFormConfig(),
          index = _this$getFormConfig3.index;
...
var script$1 = {
  ...
  computed: {
    computedSchema: function computedSchema() {
      if (this.builder.schema) return this.builder.schema.filter(function (v) {
        return !lodashEs.isEmpty(v);
      }).map(function (_ref) {
        var _ref$component = _ref.component,
            component = _ref$component === void 0 ? 'm-field' : _ref$component,
...
var script$5 = {
  ...
  methods: {
    reduceHandler: function reduceHandler(item) {
      if (lodashEs.isPlainObject(item)) return item.value;
      return item;
    }
  }
};
...
var InputMixin = {
  ...
  computed: {
    ...
    attributes: function attributes() {
      return lodashEs.omit(this.context.attributes, ['items']) || {};
    },
    hasValue: function hasValue() {
      return this.context.hasValue;
    },
    items: function items() {
      return lodashEs.get(this.context, 'attributes.items', []);
    },
    model: {
      get: function get() {
        return this.context.model;
...
var script$6 = {
  ...
  computed: {
    inline: function inline() {
      return lodashEs.isUndefined(this.attributes.inline) ? true : this.attributes.inline;
    }
  },
  data: function data() {
    return {
...
var VuexForm = /*#__PURE__*/function () {
  function VuexForm() {}

  var _proto = VuexForm.prototype;

  /**
   * Install vue formulate, and register it’s components.
   */
  _proto.install = function install(Vue, options) {
    vuexToolkit.registerComponents(Vue, {
    ...
    Object.keys(rules).forEach(function (rule) {
      return veeValidate.extend(rule, rules[rule]);
    });
...
exports.VuexForm = VuexForm$1;
exports.storeModule = index;
